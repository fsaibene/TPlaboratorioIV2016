 ...
 */
class ViajeEquipamientoVO extends Master2 {
	...
	public function getComboList2($data = null){
		$sql = 'select CONCAT_WS("/", te.tipoEquipamiento, a.marca, a.modelo, a.codigo) as label, a.idEquipamiento as data, b.idViajeEquipamiento as selected
				from equipamientos as a
				inner join tiposEquipamiento as te using (idTipoEquipamiento)
				left JOIN viajes_equipamientos as b on a.idEquipamiento = b.idEquipamiento ';
		if($data)
	...
	public function deleteDataArray($idViaje, $idEquipamientoArray = null){
	...
	public function insertDataArray($idViaje, $idEquipamientoArray){
		try{
			foreach($idEquipamientoArray as $idEquipamiento){
				$sql = 'insert ignore into '.$this->getTableName().'
						set idViaje = '.$idViaje.', idEquipamiento = '.$idEquipamiento.', idUsuarioLog = '.$_SESSION['usuarioLogueadoIdUsuario'];
				//echo($sql);
				$ro = $this->conn->prepare($sql);
				if(!$ro->execute()){
					$this->result->setStatus(STATUS_ERROR);
...
// debug zone
if($_GET['debug'] == 'ViajeEquipamientoVO' or false){
	echo "DEBUG<br>";
	$kk = new ViajeEquipamientoVO();
	//print_r($kk->getAllRows());
	$kk->idViajeEquipamiento = 116;
	$kk->usuario = 'hhh2';
