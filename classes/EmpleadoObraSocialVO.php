 ...
 */
class EmpleadoObraSocialVO extends Master2 implements iListadoExportable{
	public $idEmpleadoObraSocial = ["valor" => "",
	                   "obligatorio" => FALSE,
	                   "tipo" => "integer",
	                   "nombre" => "ID",
					];
	...
	public $poseeUpgrade = ["valor" => TRUE,
						"obligatorio" => TRUE,
						"tipo" => "bool",
						"nombre" => "posee upgrade",
	];
	public $upgradeSinec = ["valor" => TRUE,
						"obligatorio" => TRUE,
						"tipo" => "bool",
						"nombre" => "upgrade a cargo de SINEC",
	];
	...
	public function __construct()
	{
		...
		$this->setTableName('empleadosObraSocial');
		$this->setFieldIdName('idEmpleadoObraSocial');
	...
	public function getEmpleadoObraSocialPorIdEmpleado(){
...
//				$campos[] = ucfirst($campo['nombre']);//Se carga un array con los campos
	...
	public function getDataTableData($postData, $data = null){
		try{
			...
			if ($dataSql['data']) {
				foreach ($dataSql['data'] as $row) {
					$auxRow['idEmpleadoObraSocial'] = $row['idEmpleadoObraSocial'];
					...
					$auxRow['plan'] = $row['plan'];
					$auxRow['numeroAfiliado'] = $row['numeroAfiliado'];
					$auxRow['poseeUpgrade'] = $row['poseeUpgrade'] ? 'SI' : 'NO';
					$auxRow['upgradeSinec'] = $row['upgradeSinec'] ? 'SI' : 'NO';
					$auxRow['observaciones'] = limpiarCampoWysihtml5($row['observaciones']);
					$auxRow['accion'] = '<a href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoObraSocial'] . '&action=edit') . '" title="Editar"><span class="fa fa-edit fa-lg"></span></a>&nbsp;&nbsp;
                                                            <a  class="btn-compose-modal-confirm" href="#" data-href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoObraSocial'] . '&action=delete') . '" data-toggle="modal" data-target="#compose-modal-confirm" title="Eliminar"><span class="fa fa-trash-o fa-lg"></span></a>';
		...
		public function getObjectPropierties(){
			...
			$objectPropierties[] = array('nombre' => 'idEmpleadoObraSocial',  'dbFieldName' =>'idEmpleadoObraSocial',     'visibleDT' => false, 'visibleDTexport' => false,  'className' => false,    'aDataSort' => false,         'bSortable' => true,     'searchable' => false,  );   //className: Indica de que lado estara ordenado el elmento de esa columna
			...
			$objectPropierties[] = array('nombre' => 'sucursal',              'dbFieldName' =>'sucursalEstablecimiento',  'visibleDT' => true,  'visibleDTexport' => true ,  'className' => false,    'aDataSort' => false,         'bSortable' => true,     'searchable' => false,  );  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
			$objectPropierties[] = array('nombre' => 'fechaAltaEN',           'dbFieldName' =>'fechaAlta',                'visibleDT' => false, 'visibleDTexport' => false,  'className' => false,    'aDataSort' => false,         'bSortable' => true,     'searchable' => false,  );  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
			...
			$objectPropierties[] = array('nombre' => 'plan',                  'dbFieldName' =>'plan',                     'visibleDT' => false, 'visibleDTexport' => true ,  'className' => false,    'aDataSort' => false,         'bSortable' => false,    'searchable' => false,  );
			$objectPropierties[] = array('nombre' => 'número Afiliado',       'dbFieldName' =>'numeroAfiliado',           'visibleDT' => false, 'visibleDTexport' => true ,  'className' => false,    'aDataSort' => false,         'bSortable' => false,    'searchable' => false,  );
			$objectPropierties[] = array('nombre' => 'poseeUpgrade',          'dbFieldName' =>'poseeUpgrade',             'visibleDT' => false, 'visibleDTexport' => true ,  'className' => false,    'aDataSort' => false,         'bSortable' => false,    'searchable' => false,  );
			$objectPropierties[] = array('nombre' => 'upgradeSinec',          'dbFieldName' =>'upgradeSinec',             'visibleDT' => false, 'visibleDTexport' => true ,  'className' => false,    'aDataSort' => false,         'bSortable' => false,    'searchable' => false,  );
	...
	public function getSqlForTableExport($data = null){		//fc_print($data, true);
		$sql = "SELECT
				eos.idEmpleadoObraSocial,
				...
				eos.fechaAlta,
				DATE_FORMAT(eos.fechaAlta,\"%d/%m/%Y\") as fechaAltaES,
				eos.plan,
				eos.numeroAfiliado,
				eos.poseeUpgrade,
				eos.upgradeSinec,
				eos.observaciones
			FROM
				empleadosObraSocial AS eos
			LEFT JOIN empleados AS e ON e.idEmpleado = eos.idEmpleado
			LEFT JOIN sucursalesEstablecimiento AS se ON se.idSucursalEstablecimiento = eos.idSucursalEstablecimiento
...
if($_POST['action'] == 'dtSQL' && ($_POST['page'] == 'ABMempleadoObraSocial.php' || $_POST['page'] == 'ABMempleadoObraSocial2.php')){
	$aux = new EmpleadoObraSocialVO();
...
// debug zone
if($_GET['debug'] == 'EmpleadoObraSocialVO' or false){
	echo "DEBUG<br>";
	$kk = new EmpleadoObraSocialVO();
	//print_r($kk->getAllRows());
	$kk->idEmpleadoObraSocial = 116;
	$kk->usuario = 'hhh2';
