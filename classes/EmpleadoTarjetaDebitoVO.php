 ...
 */
class EmpleadoTarjetaDebitoVO extends Master2 implements iListadoExportable {
	...
	public function __construct()
	{
		...
		$this->setTableName('empleadosTarjetaDebito');
		$this->setFieldIdName('idEmpleadoTarjetaDebito');
	...
	public function getEmpleadoTarjetaDebitoPorIdEmpleado(){
...
//				$campos[] = ucfirst($campo['nombre']);//Se carga un array con los campos
		...
		try{
			...
			if ($dataSql['data']) {
				foreach ($dataSql['data'] as $row) {
					$auxRow['idEmpleadoTarjetaDebito'] = $row['idEmpleadoTarjetaDebito'];
					...
					$auxRow['tipoMarcaTarjeta'] = $row['tipoMarcaTarjeta'];
					$auxRow['nroTarjetaDebito'] = $row['nroTarjetaDebito'];
					$auxRow['fechaVencimiento'] = $row['fechaVencimiento'];
					$auxRow['fechaVencimientoES'] = $row['fechaVencimientoES'];
					$auxRow['habilitado'] = $row['habilitado'] ? 'SI' : 'NO';
					$auxRow['observaciones'] = limpiarCampoWysihtml5($row['observaciones']);
					$auxRow['accion'] = '<a  href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoTarjetaDebito'] . '&action=edit') . '" title="Editar"><span class="fa fa-edit fa-lg"></span></a>&nbsp;&nbsp;
                                                            <a  class="btn-compose-modal-confirm" href="#" data-href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoTarjetaDebito'] . '&action=delete') . '" data-toggle="modal" data-target="#compose-modal-confirm" title="Eliminar"><span class="fa fa-trash-o fa-lg"></span></a>';
	...
	public function getObjectPropierties(){
		...
		$objectPropierties[] = array('nombre' => 'idEmpleadoTarjetaDebito',     'dbFieldName' => 'idEmpleadoTarjetaDebito', 'visibleDT' => false, 'visibleDTexport' => false,   'className' => false, 'aDataSort' => false,                  'bSortable' => true, 'searchable' => false,);   //className: Indica de que lado estara ordenado el elmento de esa columna
		...
		$objectPropierties[] = array('nombre' => 'sucursal',                    'dbFieldName' => 'sucursalEstablecimiento', 'visibleDT' => true,  'visibleDTexport' => true ,   'className' => false, 'aDataSort' => false,                  'bSortable' => true, 'searchable' => true, );   //bVIsible:Indica si la columna se muestra
		$objectPropierties[] = array('nombre' => 'empresa',                     'dbFieldName' => 'tipoMarcaTarjeta',        'visibleDT' => true,  'visibleDTexport' => true ,   'className' => false, 'aDataSort' => false,                  'bSortable' => true, 'searchable' => true, );   //bVIsible:Indica si la columna se muestra
		$objectPropierties[] = array('nombre' => 'nro Tarjeta',                 'dbFieldName' => 'nroTarjetaDebito',        'visibleDT' => false, 'visibleDTexport' => true ,   'className' => false, 'aDataSort' => false,                  'bSortable' => true, 'searchable' => true, );   //bVIsible:Indica si la columna se muestra
		$objectPropierties[] = array('nombre' => 'fechaVencimientoEN',          'dbFieldName' => 'fechaVencimiento',        'visibleDT' => false, 'visibleDTexport' => false,   'className' => false, 'aDataSort' => false,                  'bSortable' => true, 'searchable' => false,);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		$objectPropierties[] = array('nombre' => 'fecha Vencimiento',           'dbFieldName' => 'fechaVencimientoES',      'visibleDT' => true,  'visibleDTexport' => true ,   'className' => false, 'aDataSort' => 'fechaVencimientoEN',   'bSortable' => true, 'searchable' => true, );  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		$objectPropierties[] = array('nombre' => 'habilitado',                  'dbFieldName' => 'habilitado',              'visibleDT' => false, 'visibleDTexport' => true ,   'className' => false, 'aDataSort' => false,                  'bSortable' => false,'searchable' => false,);
	...
	public function getDataTableProperties(){
		...
		//AASORTING
		$aaSorting = '"aaSorting": [[1,"asc"],[7, "desc"]],';
		//AASORTING
	...
	public function getSqlForTableExport($data = null){
		//fc_print($data, true);
		$sql = "SELECT
						etd.idEmpleadoTarjetaDebito,
						getEmpleado(e.idEmpleado) as empleado,
						e.nombres,
						e.apellido,
						est.establecimiento,
						se.sucursalEstablecimiento,
						tmt.tipoMarcaTarjeta,
						etd.nroTarjetaDebito,
						etd.fechaVencimiento,
					    DATE_FORMAT(etd.fechaVencimiento,\"%d/%m/%Y\") as fechaVencimientoES,
						etd.habilitado,
						etd.observaciones
					FROM
						empleadosTarjetaDebito AS etd
					LEFT JOIN empleados AS e ON e.idEmpleado = etd.idEmpleado
					LEFT JOIN sucursalesEstablecimiento AS se ON se.idSucursalEstablecimiento = etd.idSucursalEstablecimiento
					LEFT JOIN establecimientos as est on est.idEstablecimiento = se.idEstablecimiento
					LEFT JOIN tiposMarcaTarjeta AS tmt ON tmt.idTipoMarcaTarjeta = etd.idTipoMarcaTarjeta
					WHERE
						TRUE";
...
if($_POST['action'] == 'dtSQL' && ($_POST['page'] == 'ABMempleadoTarjetaDebito.php' || $_POST['page'] == 'ABMempleadoTarjetaDebito2.php')){
	$aux = new EmpleadoTarjetaDebitoVO();
...
// debug zone
if($_GET['debug'] == 'EmpleadoTarjetaDebitoVO' or false){
	echo "DEBUG<br>";
	$kk = new EmpleadoTarjetaDebitoVO();
	//print_r($kk->getAllRows());
	$kk->idEmpleadoTarjetaDebito = 116;
	$kk->usuario = 'hhh2';
