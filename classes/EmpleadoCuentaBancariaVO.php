 ...
 */
class EmpleadoCuentaBancariaVO extends Master2 implements iListadoExportable {
	public $idEmpleadoCuentaBancaria = ["valor" => "", 
	                   "obligatorio" => FALSE, 
	                   "tipo" => "integer",
	                   "nombre" => "ID",
					];
	...
	public function __construct()
	{
		...
		$this->setTableName('empleadosCuentaBancaria');
		$this->setFieldIdName('idEmpleadoCuentaBancaria');
		$this->idEmpleado['referencia'] = new EmpleadoVO();
		$this->idTipoOrigenCuentaBancaria['referencia'] = new TipoOrigenCuentaBancariaVO();
	...
	public function getEmpleadoCuentaBancariaPorIdEmpleado(){
...
//				$campos[] = ucfirst($campo['nombre']);//Se carga un array con los campos
		...
		try{
			...
			if ($dataSql['data']) {
				foreach ($dataSql['data'] as $row) {
					$auxRow['idEmpleadoCuentaBancaria'] = $row['idEmpleadoCuentaBancaria'];
					...
					$auxRow['numeroCuenta'] = $row['numeroCuenta'];
					$auxRow['cbu'] = $row['cbu'];
					$auxRow['observaciones'] = limpiarCampoWysihtml5($row['observaciones']);
					$auxRow['accion'] = '<a href="' . $data['page'] . '?' . codificarGets('id=' . $row['idEmpleadoCuentaBancaria'] . '&action=edit') . '" title="Editar"><span class="fa fa-edit fa-lg"></span></a>&nbsp;&nbsp;
                                                            <a class="btn-compose-modal-confirm" href="#" data-href="' . $postData['page'] . '?' . codificarGets('id=' . $row['idEmpleadoCuentaBancaria'] . '&action=delete') . '" data-toggle="modal" data-target="#compose-modal-confirm" title="Eliminar"><span class="fa fa-trash-o fa-lg"></span></a>';
					$auxData[] = $auxRow;
				}
	...
	public function getObjectPropierties(){
		...
		$objectPropierties[] = array('nombre' => 'idEmpleadoCuentaBancaria',  'dbFieldName' =>'idEmpleadoCuentaBancaria',   'visibleDT' => false, 'visibleDTexport' => false, 'className' => false,        'aDataSort' => false,         'bSortable' => true,     'searchable' => false);   //className: Indica de que lado estara ordenado el elmento de esa columna
		...
		$objectPropierties[] = array('nombre' => 'numero cuenta',             'dbFieldName' =>'numeroCuenta',               'visibleDT' => false, 'visibleDTexport' => true ,   'className' => false,      'aDataSort' => false,         'bSortable' => true,     'searchable' => true);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		$objectPropierties[] = array('nombre' => 'cbu',                       'dbFieldName' =>'cbu',                        'visibleDT' => false, 'visibleDTexport' => true ,   'className' => false,      'aDataSort' => false,         'bSortable' => true,     'searchable' => true);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		$objectPropierties[] = array('nombre' => 'observaciones',             'dbFieldName' =>'observaciones',              'visibleDT' => false, 'visibleDTexport' => true ,   'className' => false,      'aDataSort' => false,         'bSortable' => true,     'searchable' => true);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
	...
	public function getSqlForTableExport($data = null){
		//fc_print($data, true);
		$sql = "SELECT
						ecb.idEmpleadoCuentaBancaria,
						ecb.idEmpleado,
						getEmpleado(e.idEmpleado) as empleado,
						e.apellido,
						e.nombres,
						ecb.idSucursalEstablecimiento,
						se.sucursalEstablecimiento,
						es.establecimiento,
						ecb.idTipoOrigenCuentaBancaria,
						ecb.fechaAlta,
						DATE_FORMAT(ecb.fechaAlta,\"%d/%m/%Y\") as fechaAltaES,
						ecb.numeroCuenta,
						ecb.cbu,
						ecb.observaciones
					FROM
						empleadosCuentaBancaria AS ecb
					LEFT JOIN empleados AS e ON e.idEmpleado = ecb.idEmpleado
					LEFT JOIN sucursalesEstablecimiento AS se ON se.idSucursalEstablecimiento = ecb.idSucursalEstablecimiento
					LEFT JOIN tiposOrigenCuentaBancaria AS toc ON toc.idTipoOrigenCuentaBancaria = ecb.idTipoOrigenCuentaBancaria
...
if($_POST['action'] == 'dtSQL' && ($_POST['page'] == 'ABMempleadoCuentaBancaria.php' || $_POST['page'] == 'ABMempleadoCuentaBancaria2.php')){
	$aux = new EmpleadoCuentaBancariaVO();
...
// debug zone
if($_GET['debug'] == 'EmpleadoCuentaBancariaVO' or false){
	echo "DEBUG<br>";
	$kk = new EmpleadoCuentaBancariaVO();
	//print_r($kk->getAllRows());
	$kk->idEmpleadoCuentaBancaria = 116;
	$kk->usuario = 'hhh2';
