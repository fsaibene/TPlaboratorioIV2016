 ...
 */
class EmpleadoSeguroVO extends Master2  implements iListadoExportable {
	public $idEmpleadoSeguro = ["valor" => "", 
	                   "obligatorio" => FALSE, 
	                   "tipo" => "integer",
	                   "nombre" => "ID",
					];
	...
	public $archivo = ["valor" => "",
		...
		"tipo" => "string",
		"nombre" => "archivo",
		"ruta" => "empleados/empleadosSeguros/", // de files/ en adelante
	...
	public function __construct()
	{
		...
		$this->setTableName('empleadosSeguros');
		$this->setFieldIdName('idEmpleadoSeguro');
		$this->idEmpleado['referencia'] = new EmpleadoVO();
		$this->idTipoSeguro['referencia'] = new TipoSeguroVO();
		$this->idTipoCoberturaSeguro['referencia'] = new TipoCoberturaSeguroVO();
	...
	public function getSegurosProximosAVencer() {
		$sql = 'select CONCAT(a.apellido, ", ", a.nombres) as empleado, tipoSeguro, DATEDIFF(fechavencimiento, NOW()) as diasFaltantes
				from empleados as a
				inner join (
					select max(fechaHasta) as fechaVencimiento, tipoSeguro, idEmpleado
					from empleadosSeguros
					INNER JOIN tiposSeguro as x using (idTipoSeguro)
					group by tipoSeguro, idEmpleado
...
//				$campos[] = ucfirst($campo['nombre']);//Se carga un array con los campos
		...
		try{
			...
			if ($dataSql['data']) {
				foreach ($dataSql['data'] as $row) {
					$auxRow['idEmpleadoSeguro'] = $row['idEmpleadoSeguro'];
					...
					$auxRow['tipoSeguro'] = $row['tipoSeguro'];
					$auxRow['tipoCoberturaSeguro'] = $row['tipoCoberturaSeguro'];
					...
					$auxRow['monto'] = $row['monto'];
					$auxRow['numeroPoliza'] = $row['numeroPoliza'];
					...
					$auxRow['accion'] .= '<a  href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoSeguro'] . '&action=edit') . '" title="Editar"><span class="fa fa-edit fa-lg"></span></a>&nbsp;&nbsp;
                                                            <a  class="btn-compose-modal-confirm" href="#" data-href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoSeguro'] . '&action=delete') . '" data-toggle="modal" data-target="#compose-modal-confirm" title="Eliminar"><span class="fa fa-trash-o fa-lg"></span></a>';
	...
	public function getSqlForTableExport($data = null){	
		...
		$sql = "SELECT
						...
						ts.tipoSeguro,
						tcs.tipoCoberturaSeguro,
						es.fechaDesde,
						DATE_FORMAT(es.fechaDesde,\"%d/%m/%Y\") as fechaDesdeES,
						es.fechaHasta,
						DATE_FORMAT(es.fechaHasta,\"%d/%m/%Y\") as fechaHastaES,
						es.monto,
						es.numeroPoliza,
						es.archivo,
						es.observaciones
					FROM
						empleadosSeguros AS es
					...
					LEFT JOIN tiposSeguro AS ts ON ts.idTipoSeguro = es.idTipoSeguro
					LEFT JOIN tiposCoberturaSeguro AS tcs ON tcs.idTipoCoberturaSeguro = es.idTipoCoberturaSeguro
	...
	public function getObjectPropierties(){
		...
		$objectPropierties[] = array('nombre' => 'idEmpleadoSeguro',         'dbFieldName' => 'idEmpleadoSeguro',        'visibleDT' => false, 'visibleDTexport' => false, 'className' => false,       'aDataSort' => false,         'bSortable' => true,     'searchable' => false);   //className: Indica de que lado estara ordenado el elmento de esa columna
		...
		$objectPropierties[] = array('nombre' => 'sucursal Establecimiento', 'dbFieldName' => 'sucursalEstablecimiento', 'visibleDT' => false, 'visibleDTexport' => true , 'className' => false,       'aDataSort' => false,         'bSortable' => true,     'searchable' => true);   //bVIsible:Indica si la columna se muestra
		$objectPropierties[] = array('nombre' => 'tipo de Seguro',           'dbFieldName' => 'tipoSeguro',              'visibleDT' => true,  'visibleDTexport' => true , 'className' => false,       'aDataSort' => false,         'bSortable' => true,     'searchable' => true);   //bVIsible:Indica si la columna se muestra
		$objectPropierties[] = array('nombre' => 'tipo de Cobertura',        'dbFieldName' => 'tipoCoberturaSeguro',     'visibleDT' => false, 'visibleDTexport' => true , 'className' => false,       'aDataSort' => false,         'bSortable' => true,     'searchable' => true);   //bVIsible:Indica si la columna se muestra
		...
		$objectPropierties[] = array('nombre' => 'fecha Desde',              'dbFieldName' => 'fechaDesdeES',            'visibleDT' => true,  'visibleDTexport' => true , 'className' => false,       'aDataSort' => 'fechaDesdeEN','bSortable' => true,     'searchable' => true);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		...
		$objectPropierties[] = array('nombre' => 'fecha Hasta',              'dbFieldName' => 'fechaHastaES',            'visibleDT' => true,  'visibleDTexport' => true , 'className' => false,       'aDataSort' => 'fechaHastaEN','bSortable' => true,     'searchable' => true);  //sercheable: indica si se podra buscar mediante el textbox de busqueda este campo
		$objectPropierties[] = array('nombre' => 'monto',                    'dbFieldName' => 'monto',                   'visibleDT' => false, 'visibleDTexport' => true , 'className' => false,       'aDataSort' => false,         'bSortable' => false,    'searchable' => false);
		$objectPropierties[] = array('nombre' => 'número de Poliza',         'dbFieldName' => 'numeroPoliza',            'visibleDT' => false, 'visibleDTexport' => true , 'className' => false,       'aDataSort' => false,         'bSortable' => false,    'searchable' => false);
...
if($_POST['action'] == 'dtSQL' && ($_POST['page'] == 'ABMempleadoSeguro.php' || $_POST['page'] == 'ABMempleadoSeguro2.php')){
	$aux = new EmpleadoSeguroVO();
...
// debug zone
if($_GET['debug'] == 'EmpleadoSeguroVO' or false){
	echo "DEBUG<br>";
	$kk = new EmpleadoSeguroVO();
	//print_r($kk->getAllRows());
	$kk->idEmpleadoSeguro = 116;
	$kk->usuario = 'hhh2';
