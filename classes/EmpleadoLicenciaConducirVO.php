 ...
 */
class EmpleadoLicenciaConducirVO extends Master2 implements  iListadoExportable{
	public $idEmpleadoLicenciaConducir = ["valor" => "", 
	                   "obligatorio" => FALSE, 
	                   "tipo" => "integer",
	                   "nombre" => "ID",
					];
	...
	public $archivo = ["valor" => "",
		...
		"tipo" => "string",
		"nombre" => "archivo",
		"ruta" => "empleados/empleadosLicenciasConducir/", // de files/ en adelante
	...
	public function __construct()
	{
		...
		$this->setTableName('empleadosLicenciasConducir');
		$this->setFieldIdName('idEmpleadoLicenciaConducir');
		$this->idEmpleado['referencia'] = new EmpleadoVO();
		$this->idTipoCategoriaLicenciaConducir['referencia'] = new TipoCategoriaLicenciaConducirVO();
	}
	...
	public function getLicenciasConducirProximosAVencer() {
		$sql = 'select CONCAT(a.apellido, ", ", a.nombres) as empleado, DATEDIFF(fechavencimiento, NOW()) as diasFaltantes
				...
				inner join (
					...
					from empleadosLicenciasConducir
					group by idEmpleado
...
//				$campos[] = ucfirst($campo['nombre']);//Se carga un array con los campos
		...
		try{
			...
			if ($dataSql['data']) {
				foreach ($dataSql['data'] as $row) {
					$auxRow['idEmpleadoLicenciaConducir'] = $row['idEmpleadoLicenciaConducir'];
					$auxRow['empleado'] = $row['apellido'].', '.$row['nombres'];
					$auxRow['apellido'] = $row['apellido'];
					$auxRow['nombres'] = $row['nombres'];
					$auxRow['tipoCategoriaLicenciaConducir'] = $row['tipoCategoriaLicenciaConducir'];
					...
					$auxRow['accion'] = '';
					if($auxRow['archivo']){
						$auxRow['accion'] ='<a target="_blank" href="'.getPath().'/files/empleados/empleadosLicenciasConducir/'.$row['archivo']. '" title="Descargar documento"><span class="fa fa-file-o fa-lg"></span></a>&nbsp;&nbsp';
					}
					$auxRow['accion'] .= '<a href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoLicenciaConducir'] . '&action=edit') . '" title="Editar"><span class="fa fa-edit fa-lg"></span></a>&nbsp;&nbsp;
                                                            <a  class="btn-compose-modal-confirm" href="#" data-href="'.$postData['page'].'?' . codificarGets('id=' . $row['idEmpleadoLicenciaConducir'] . '&action=delete') . '" data-toggle="modal" data-target="#compose-modal-confirm" title="Eliminar"><span class="fa fa-trash-o fa-lg"></span></a>';
					$auxData[] = $auxRow;
				}
	...
	public function getObjectPropierties(){
		...
		$objectPropierties[] = array('nombre' => 'idEmpleadoLicenciaConducir', 'dbFieldName' => 'idEmpleadoLicenciaConducir',    'visibleDT' => false, 'visibleDTexport' => false,  'className' => false,        'aDataSort' => false,         'bSortable' => true,     'searchable' => false);   //className: Indica de que lado estara ordenado el elmento de esa columna
		...
		$objectPropierties[] = array('nombre' => 'categoría',                  'dbFieldName' => 'tipoCategoriaLicenciaConducir', 'visibleDT' => true,  'visibleDTexport' => true ,  'className' => false,        'aDataSort' => false,         'bSortable' => true,     'searchable' => true);   //bVIsible:Indica si la columna se muestra
	...
	public function getSqlForTableExport($data = null){
		...
		$sql = "SELECT
						elc.idEmpleadoLicenciaConducir,
						getEmpleado(e.idEmpleado) as empleado,
					 	e.nombres,
						e.apellido,
						tclc.tipoCategoriaLicenciaConducir,
						elc.fechaDesde,
						DATE_FORMAT(elc.fechaDesde,\"%d/%m/%Y\") as fechaDesdeES,
						elc.fechaHasta,
						DATE_FORMAT(elc.fechaHasta,\"%d/%m/%Y\") as fechaHastaES,
						elc.archivo,
						elc.observaciones
					FROM
						empleadosLicenciasConducir AS elc
					LEFT JOIN empleados AS e ON e.idEmpleado = elc.idEmpleado
					LEFT JOIN tiposCategoriaLicenciaConducir AS tclc ON tclc.idTipoCategoriaLicenciaConducir = elc.idTipoCategoriaLicenciaConducir
					WHERE
						TRUE";
...
if($_POST['action'] == 'dtSQL' && ($_POST['page'] == 'ABMempleadoLicenciaConducir.php' || $_POST['page'] == 'ABMempleadoLicenciaConducir2.php')){
	$aux = new EmpleadoLicenciaConducirVO();
...
// debug zone
if($_GET['debug'] == 'EmpleadoLicenciaConducirVO' or false){
	echo "DEBUG<br>";
	$kk = new EmpleadoLicenciaConducirVO();
	//print_r($kk->getAllRows());
	$kk->idEmpleadoLicenciaConducir = 116;
	$kk->usuario = 'hhh2';
